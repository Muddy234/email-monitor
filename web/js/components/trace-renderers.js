/**
 * Shared pipeline trace stage renderers.
...
export function traceStage(label, status, content, isLast = false) {
    const dotClass = {
        active: "",
...
// ─── Stage 1: Raw Email ──────────────────────────────────────
export function renderStage1(email, bodyPreview) {
...
// ─── Stage 2: Filter ─────────────────────────────────────────
export function renderStage2(email, cls, wasFiltered) {
    if (wasFiltered) {
...
export function renderStage3_signals(evt, email, wasFiltered) {
    if (wasFiltered) {
...
// ─── Stage 4: Context ────────────────────────────────────────
export function renderStage4_context(contact, evt, email, wasFiltered) {
    if (wasFiltered) {
    ...
    if (!contact && !evt) {
        if (email.status === Status.ACTIVE || email.status === Status.PENDING) {
...
// ─── Stage 5: Draft ──────────────────────────────────────────
export function renderStage5_draft(draft, evt, cls, wasFiltered) {
    if (wasFiltered) {
...
// ─── Stage 6: Delivery ───────────────────────────────────────
export function renderStage6_delivery(draft) {
    if (!draft) {
