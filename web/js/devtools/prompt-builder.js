...
7. Useful response — Given all of the above, what type of reply would be most helpful and move things forward?
   Diagnostic question check (MANDATORY if Step 4 flagged central gaps): Before committing to scaffold, you MUST answer this question: Is there ONE question — directed at the sender or a third party — whose answer would determine the correct course of action?
   How to find the diagnostic question: The sender described a situation and asked for a decision. Ask yourself: what is the ONE fact that, if known, would make the decision obvious? Often, the sender has described WHAT is happening (symptoms) without explaining WHY or HOW it started (cause). A symptom explanation is not a complete diagnosis. If the origin, intent, or triggering event behind the situation is unknown, that is the diagnostic question — ask it. Common diagnostic questions ask about:
   ...
   Decision: If such a question exists → this is a DIAGNOSTIC draft, not a scaffold. Write a short reply that (a) acknowledges the situation, (b) asks the targeted question, and (c) gives a conditional decision ("if X, then do Y; if Z, then do W"). A 3-5 sentence reply that resolves ambiguity with one question is almost always more useful than deferring to the user via scaffold blocks. Only offer conditional decisions on matters consistent with the user's apparent authority level as reflected in their behavioral profile and role.
   If no single question can resolve the gap → this is a SCAFFOLD draft. Proceed with scaffold format.
   Conclude Step 7 by stating your verdict: "VERDICT: diagnostic" or "VERDICT: scaffold".
