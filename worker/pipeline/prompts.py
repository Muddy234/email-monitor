...
7. Useful response — Given all of the above, what type of reply would be most helpful and move things forward?
   ...
   Decision:
   - If such a question exists → VERDICT: diagnostic. Write a short reply that (a) acknowledges the situation, (b) asks the targeted question, and (c) gives a conditional decision ("if X, then do Y; if Z, then do W"). A 3-5 sentence reply that resolves ambiguity with one question is almost always more useful than deciding without context or deferring to the user via scaffold blocks. Only offer conditional decisions on matters consistent with the user's apparent authority level as reflected in their behavioral profile and role.
