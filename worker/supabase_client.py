...
class SupabaseWorkerClient:
    ...
    def update_calibration_status(self, user_id, status, rules=None):
        """Update a user's calibration status and optional rules.

        Args:
            user_id: UUID string.
            status: One of 'pending', 'running', 'passed', 'needs_review'.
            rules: Optional correction rules text to store.
        """
        data = {"calibration_status": status}
        if status == "passed":
            data["calibrated_at"] = datetime.utcnow().isoformat()
        if rules is not None:
            data["calibration_rules"] = rules
        self.client.table("profiles").update(data).eq("id", user_id).execute()

    def update_calibration_iteration(self, user_id, iteration):
        """Update the current calibration iteration number."""
        self.client.table("profiles").update(
            {"calibration_iteration": iteration}
        ).eq("id", user_id).execute()

    def store_calibration_results(self, rows):
        """Insert calibration result rows.

        Args:
            rows: list[dict] with keys matching calibration_results table.
        """
        if not rows:
            return
        for i in range(0, len(rows), 100):
            self.client.table("calibration_results").insert(
                rows[i:i + 100]
            ).execute()

    def fetch_calibration_rules(self, user_id):
        """Fetch stored calibration rules for a user.

        Returns:
            str or None: The calibration rules text.
        """
        result = (
            self.client.table("profiles")
            .select("calibration_rules")
            .eq("id", user_id)
            .maybe_single()
            .execute()
        )
        if result.data:
            return result.data.get("calibration_rules")
        return None
